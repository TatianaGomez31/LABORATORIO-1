...
result:  .asciiz "El número mayor es:"        # Mensaje para mostrar el resultado (el mayor número)
...
main:
    ...
    # Encontrar el número mayor entre los tres
    move $t3, $t0           # Asume que el primer número ($t0) es el mayor y lo guarda en $t3
    ble $t1, $t3, check_t1  # Si $t1 no es mayor que $t3, salta a la etiqueta check_t1
    move $t3, $t1           # Si $t1 es mayor que $t3, actualiza $t3 con el valor de $t1
    
check_t1:
    ble $t2, $t3, check_t2  # Si $t2 no es mayor que $t3, salta a la etiqueta check_t2
    move $t3, $t2           # Si $t2 es mayor que $t3, actualiza $t3 con el valor de $t2
    
check_t2:
    # Imprimir el resultado
    li $v0, 4              # Llamada al sistema para imprimir una cadena (código 4)
    la $a0, result          # Carga la dirección del mensaje "El número mayor es:" en $a0
    syscall                 # Imprime el mensaje

    li $v0, 1              # Llamada al sistema para imprimir un entero (código 1)
    move $a0, $t3           # Carga el valor del número mayor en $a0 para su impresión
    syscall                 # Imprime el valor del número mayor
